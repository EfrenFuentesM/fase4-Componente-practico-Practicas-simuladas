...
from software_fj.gui.estilos import C, F, aplicar_estilos, SidebarNavBtn
...
class SoftwareFJApp(tk.Tk):
    ...
    def _crear_sidebar(self):
        ...
        # Generación dinámica de botones de menú (con esquinas redondeadas)
        for key, lbl in self._NAV:
            nav_btn = SidebarNavBtn(
                sidebar, lbl, command=lambda k=key: self._navegar(k)
            )
            nav_btn.pack(fill="x", padx=8, pady=2)
            self._nav_btns[key] = nav_btn
    ...
    # _hover_on / _hover_off ya no son necesarios:
    # SidebarNavBtn gestiona hover y estado activo internamente.
    ...
    def _navegar(self, vista: str):
        ...
        """
        # Actualizar el estado activo de cada botón del sidebar
        for key, nav_btn in self._nav_btns.items():
            nav_btn.set_active(key == vista)
